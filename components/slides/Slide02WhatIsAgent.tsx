  ...
  return (
    <div className="slide" style={{ justifyContent: "center" }}>
      <div className="relative z-10" style={{ maxWidth: "900px" }}>
        <motion.p {...fade(0.05)} style={{ fontFamily: "'Heebo'", fontSize: "12px", fontWeight: 700, letterSpacing: "0.22em", color: "var(--text-muted)", textTransform: "uppercase", marginBottom: "20px" }}>
          מושג בסיס
        </motion.p>

        <motion.h2 {...fade(0.15)} style={{ fontSize: "58px", fontWeight: 700, lineHeight: 1.1, marginBottom: "32px" }}>
          מה זה בכלל<br />
        ...
        <motion.div {...fade(0.28)} className="highlight-box" style={{ marginBottom: "32px", padding: "20px 24px" }}>
          <p style={{ fontSize: "15px", fontFamily: "'JetBrains Mono'", color: "rgba(167,139,250,0.6)", letterSpacing: "0.1em", marginBottom: "10px", textTransform: "uppercase" }}>
            הסבר לבן 4
          </p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          <motion.div {...fade(0.4)} className="card" style={{ borderTop: "2px solid var(--border)" }}>
            <p style={{ fontSize: "11px", fontFamily: "'Heebo'", fontWeight: 700, letterSpacing: "0.18em", color: "var(--text-muted)", textTransform: "uppercase", marginBottom: "14px" }}>ChatGPT רגיל</p>
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {["שואל → עונה → נגמר", "שוכח הכל אחרי השיחה", "לא יכול לעשות דברים", "תלוי בך לספק הכל"].map((item, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <span style={{ fontSize: "17px", color: "rgba(238,242,247,0.65)" }}>{item}</span>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div {...fade(0.5)} className="card" style={{ borderTop: "2px solid var(--primary)" }}>
            <p style={{ fontSize: "11px", fontFamily: "'Heebo'", fontWeight: 700, letterSpacing: "0.18em", color: "var(--primary-light)", textTransform: "uppercase", marginBottom: "14px" }}>AI Agent</p>
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              {["חושב בלולאה — צועד אחרי צעד", "זוכר בין שיחות", "יכול לקרוא קבצים, לשלוח הודעות, לגלוש", "פועל לבד על סמך מה שקרה"].map((item, i) => (
                <div key={i} style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <span style={{ fontSize: "17px", color: "rgba(238,242,247,0.85)" }}>{item}</span>
                </div>
              ))}
            </div>
          </motion.div>
